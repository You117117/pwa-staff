 ...
 * - enlève "En attente : 0"
 * - ajoute une pastille .status-chip avec "Vide"
 * - le reste du comportement est conservé
 */
(() => {
  ...
  // on garde une copie des tables pour le filtre
  let lastTables = [];
  ...
  // mise en forme du grid si présent
  try {
  ...
  function renderTables(data){
    ...
    lastTables = list; // on garde

    tablesWrap.innerHTML = '';
    const onlyPending = (filterSel?.value === 'pending');

    // notre requête peut toujours renvoyer pending, on peut filtrer dessus,
    // mais on ne l’affiche plus dans le badge
    ...
    filtered.forEach(t => {
      ...
      // ⬇️ ICI : plus de "En attente : 0", on met le badge de statut
      el.innerHTML = `
        <div class="row">
