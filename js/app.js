...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const chime = {
    ...
    playWebAudio() {
      ...
      g.gain.exponentialRampToValueAtTime(0.2, t0 + 0.4);
  ...
  function generateChimeWavUrl() {
    ...
    for (let i = 0; i < len; i++) {
      const t = i / sr;
      let f = seq[seq.length - 1].f;
      for (let j = 0; j < seq.length; j++) {
        if (t >= seq[j].t) f = seq[j].f;
      }
  ...
  function loadState() {
    try {
      ...
      if (s.tableMemory)
        Object.entries(s.tableMemory).forEach(
          ([tid, v]) =>
            (tableMemory[tid] = {
              isClosed: !!v.isClosed,
      ...
      if (s.autoBuffer)
        Object.entries(s.autoBuffer).forEach(
          ([tid, v]) => (autoBuffer[tid] = { until: v.until }),
        );
      if (s.payClose)
        Object.entries(s.payClose).forEach(
          ([tid, v]) => (payClose[tid] = { closeAt: v.closeAt }),
        );
      if (s.alertedTickets)
        Object.entries(s.alertedTickets).forEach(
          ([tid, arr]) => (alertedTickets[tid] = new Set(arr || [])),
        );
  ...
  function renderTables(tables) {
    ...
    sorted.forEach((table) => {
      ...
      const timeLabel = localLastActivity[id]
